...
(defun tramp-aware-make-temp-file (prefix)
  "Create a temporary file on the remote host identified by VEC.
Return the local name of the temporary file."
  (if (not (tramp-tramp-file-p default-directory))
      (make-temp-file prefix)
    (let ((prefix (expand-file-name
                   prefix (tramp-get-remote-tmpdir (tramp-dissect-file-name default-directory))))
          result)
      (while (not result)
...
(defun python-send-region (start end)
  ...
  ;; non-ASCII.
  (interactive "r")
  (let* ((f (tramp-aware-make-temp-file "py"))
         (f-local (if (tramp-tramp-file-p f) (with-parsed-tramp-file-name f nil localname) f))
	 (command
